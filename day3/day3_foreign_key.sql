...
-- using the same data from 02_insertdata :)
INSERT INTO users_detail (name, email, gender, date_of_birth, salary) VALUES
...
--  Let’s create an addresses table where each address belongs to a users_detail.
-- addresses table from 03_foreign key

set autocommit = 0; 
 -- Adding ON DELETE Action ----
 -- CASCADE:          --  If you want addresses to be automatically deleted when the user is deleted
 ALTER TABLE addresses
 ADD CONSTRAINT fk_user_details FOREIGN KEY (user_id) REFERENCES users_detail(id);
...
-- SET NULL :      -- If you want addresses to be NULL when the user is deleted
 ALTER TABLE addresses
 ADD CONSTRAINT fk_null FOREIGN KEY (user_id) REFERENCES users_detail(id) ON DELETE SET NULL;
 DELETE FROM addresses
 WHERE id = 13;
 

-- ON DELETE RESTRICT / NO ACTION / RESTART : -- YOU CAN NOT DELETE A PARENT ROW IF A CHILD ROW IS USING IT 
ALTER TABLE addresses
ADD CONSTRAINT fk_res FOREIGN KEY (user_id) REFERENCES users_detail(id) ON DELETE RESTRICT;
 DELETE FROM users_detail 
 WHERE id = 7;
